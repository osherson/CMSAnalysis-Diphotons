...
abin_edges = [0.003, 0.00347, 0.00395, 0.00444, 0.00494, 0.00545, 0.00597, 0.0065, 0.00704, 0.00759, 0.00815, 0.0093, 0.01049, 0.03]

signalCoupling = 9
abin = int(sys.argv[1])
sigHist = ROOT.TH1D("sigHist","sigHist;;",len(signal_mjj)-1,array("d",signal_mjj))
sig=GetSignalHist(sigHist, abin, lumi, signalCoupling)
sigHist.SetName(sig)
fhists = {}
...
l.SetTextSize(0.040)
#l.SetTextColor(12)
l.DrawLatex(0.19,0.84,"%s#leq #alpha^{reco}<%s"%(alphaLowEdge, alphaHiEdge))

leg_x1 = 0.6
leg = ROOT.TLegend(leg_x1,0.40,0.94,0.94)
leg.SetTextFont(42)
...
#leg2 = ROOT.TLegend(0.7,0.65,0.89,0.94)
...
#leg.AddEntry(blank1, "Functional Fit:")
leg.AddEntry(blank1, "")
...
#l.SetTextColor(12)
ll.DrawLatex(leg_x1 + 0.01, 0.85,"Functional Fit: ")

for ff in drawOrder:
  if(ff not in fhists.keys()): continue
  fhist = fhists[ff]
  #fhist.SetLineColor(lcolors[ff])
...
leg.AddEntry(sigHist,"X #rightarrow #phi#phi #rightarrow (#gamma#gamma)(#gamma#gamma)")
leg.AddEntry(blank1, "m_{X} = 1.2 TeV")
leg.AddEntry(blank1,"m_{\phi} = 6 GeV")
leg.AddEntry(blank1,"(m_{X}N)/f = 1/ %i"%signalCoupling)
...
for (ff,(lr,resid)) in resids.items():
  ...
  # paper
  resid.GetXaxis().SetTitle('Di-Cluster mass [TeV]')
  resid.SetTitle("")
  resid.SetStats(0)
...
cl.SetTextAlign(11)
cl.SetNDC()
cl.DrawLatex(0.58, 0.84, "#chi^{{2}} / ndf = {0:.2f} / {1:d} = {2:.2f}".format(
