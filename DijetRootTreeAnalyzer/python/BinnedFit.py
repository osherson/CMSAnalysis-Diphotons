...
if __name__ == '__main__':
    ...
    # paper:
    myRebinnedDensityTH1.GetYaxis().SetTitle('d#sigma/dm_{4j} [pb/TeV]')
    # PAS:
    ...
    if 'PF' in box:
        myRebinnedDensityTH1.SetMaximum(2e-02)#20
        myRebinnedDensityTH1.SetMinimum(2e-11)#2e-8
    elif 'Calo' in box:
        myRebinnedDensityTH1.SetMaximum(2e3)
        if w.var('mjj').getMax() > 1246:            
            #myRebinnedDensityTH1.SetMaximum(10)
            #myRebinnedDensityTH1.SetMinimum(2e-8)
	    myRebinnedDensityTH1.SetMaximum(10)
            myRebinnedDensityTH1.SetMinimum(5e-9)
        else:
    ...
    if 'Calo' in box:
        ...
        pave_sel.AddText("AK4 CHS Jets")
        #pave_sel.AddText("Wide PF-jets")        
        #pave_sel.AddText("%.1f < m_{jj} < %.1f TeV"%(w.var('mjj').getMin('Low')/1000.,w.var('mjj').getMax('High')/1000.))
        pave_sel.AddText("m_{2j} > %.2f TeV"%(w.var('mjj').getMin('Low')/1000.))
    #pave_sel.AddText("|#eta| < 2.5, |#Delta#eta| < 1.1")
    pave_sel.AddText("|#eta| < 2.5, |#Delta#eta| < 1.1, M_{asym}<0.1, #DeltaR_{1,2}<2")
    pave_sel.Draw("SAME")
