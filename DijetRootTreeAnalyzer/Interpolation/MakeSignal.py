...
class HC:
    ...
    def morph(self, M, N, scaled=False):
        ...
        print(self._lowI, self._hiI)

        HL = self._histArr[self._lowI].Clone("tempHL_%d" % M)
        HH = self._histArr[self._hiI].Clone("tempHH_%d" % M)
        print("Hist array: ", self._histArr[self._lowI], self._histArr[self._hiI])
        print("Mass Array: ", self._massArr[self._lowI], self._massArr[self._hiI])
...
def signalmaker(o):
    ...
    for m in STEPARR:
        ...
        TREES = ["nominal"] if o.TREE is None else [str(o.TREE)]
