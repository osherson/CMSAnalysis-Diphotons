...
def WriteAlphaEff_sd(signal, anum, eff, sdir):
  oFile = open("{}/alphaFraction_alpha{}_{}_sd.txt".format(sdir,anum,sig), "w")
  #print(anum, eff)
  oFile.write(str(eff))
  oFile.close()
  return
...
def WriteTotalEff_sd(signal, anum, aeff, sdir):
  ceff_file = open("../inputs/Shapes_fromInterpo/unBinned/{}/{}.txt".format(sig,sig),"r")
  ceff = float(ceff_file.readlines()[0])
  teff = ceff * aeff

  oFile = open("{}/{}_sd.txt".format(sdir,signal),"w")
  oFile.write(str(teff))
  oFile.close()
  return
...
for sig in os.listdir(int_dir):
  ...
  for abin in range(len(AlphaBins)-1):
    ...
    ahist_sd.Smooth()
    lAb_sd = ahist_sd.FindBin(lA)
    hAb_sd = ahist_sd.FindBin(hA)
    aI_sd = ahist_sd.Integral(lAb_sd,hAb_sd)
    aI_sd = max(0.0, aI_sd)
    #c1 = ROOT.TCanvas()
    #c1.cd()
    #ahist_sd.GetXaxis().SetRangeUser(lA,hA)
    #ahist_sd.Draw()
    #c1.Print("temp.png")
    #print(ahist_sd.Integral())
    #print(lAb_sd,hAb_sd)
    #print(aI_sd, tI_sd)
    frac_sd = aI_sd / tI_sd

    print("{}, {:4f}, {:.4f}, {:.4f}".format(abin, frac*100, frac_sd*100, abs(frac_sd - frac)/frac * 100))
    ...
    WriteAlphaEff_sd(sig,abin,frac_sd,thisdir)
    WriteTotalEff_sd(sig,abin,frac_sd,thisdir)
