...
gen_signal_dir = "./inputs/"
int_signal_dir = "./inputs/Shapes_fromInterpo/"
alpha = 0.005
...
for sub, dirs, files in os.walk(gen_signal_dir):
  for dd in dirs:
    ...
    elif("{}X".format(int_signal_dir) in os.path.join(sub,dd)):
      xm = int(dd[1: dd.find("A")])
      phim = float(dd[dd.find("A")+1 :].replace("p",".") )
      if(phim / xm == alpha):
...
for sig, sdir in gen_signals.items():
  ...
  eff = getEff(sig,sdir)
  #print(eff / 1000)
  os.system("python python/DiphotonCardMakerSingle.py -f dijet -l 5.99 -s {} -x {}".format(sig, eff/1000))
  os.system("combine output/dijet_combine_gg_{}_lumi-5.990_diphoton_dijet.txt -M AsymptoticLimits".format(xm))
  os.system("mv higgsCombineTest.AsymptoticLimits.mH120.root combineOutput/2018/X{}.root".format(xm))
