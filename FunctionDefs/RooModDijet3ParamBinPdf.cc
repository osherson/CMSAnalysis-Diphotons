...
#include "../interface/RooModDijet3ParamBinPdf.h"
...
ClassImp(RooModDijet3ParamBinPdf)
//---------------------------------------------------------------------------
RooModDijet3ParamBinPdf::RooModDijet3ParamBinPdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
...
//---------------------------------------------------------------------------
RooModDijet3ParamBinPdf::RooModDijet3ParamBinPdf(const RooModDijet3ParamBinPdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooModDijet3ParamBinPdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooModDijet3ParamBinPdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooModDijet3ParamBinPdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooModDijet3ParamBinPdf::evaluate() const
{
  ...
  ModDijet3ParamFunction func;
  double params[5];
  params[0] = sqrts;    params[1] = p1;
  params[2] = p2;
  params[3] = meff;     params[4] = seff;
...
// //---------------------------------------------------------------------------
Int_t RooModDijet3ParamBinPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooModDijet3ParamBinPdf::analyticalIntegral(Int_t code, const char* rangeName) const{
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
	...
	for (Int_t iBin=0; iBin<xBins; iBin++){
     ...
     cout << "WARNING IN RooModDijet3ParamBinPdf: integration code is not correct" << endl;
