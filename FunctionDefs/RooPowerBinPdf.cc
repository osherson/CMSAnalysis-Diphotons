...
#include "../interface/RooPowerBinPdf.h"
...
ClassImp(RooPowerBinPdf)
//---------------------------------------------------------------------------
RooPowerBinPdf::RooPowerBinPdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
...
//---------------------------------------------------------------------------
RooPowerBinPdf::RooPowerBinPdf(const RooPowerBinPdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooPowerBinPdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooPowerBinPdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooPowerBinPdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooPowerBinPdf::evaluate() const
{
  ...
  if (integral>0.0) {
    ...
    std::cout << "Integral less than 0; int = " << integral << std::endl;  
    return 0;
  }
...
// //---------------------------------------------------------------------------
Int_t RooPowerBinPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooPowerBinPdf::analyticalIntegral(Int_t code, const char* rangeName) const{
  ...
  std::cout << "In analytical integral function" << std::endl;
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooPowerBinPdf: integration code is not correct" << endl;
