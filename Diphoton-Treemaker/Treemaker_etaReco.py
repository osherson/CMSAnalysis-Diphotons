...
saveTreeFolder = "/cms/sclark-2/DiPhotonsTrees/etaReco/" 
...
def Treemaker(folder, Dataset, isData, year):
    ...
    for name in files:
      ...
      if (File.endswith(".root") and "flat" in File):
        if(os.path.getsize(File) > 100):
          ...
          #if(fcount < 3000): 
          if(fcount >= 3000 and fcount < 6000):
          #if(fcount >= 6000): #For testing
  ...
  for b in Branches:
    ...
    Rdf = Rdf.Define("nJets", "return jet_energy_b.size();")
    ...
    Rdf = Rdf.Define("nj_energy", "getClosestJetEn(ruclu_eta, ruclu_phi, ruclu_energy, moe, jet_pt_b, jet_eta_b, jet_phi_b, jet_mass_b, jet_energy_b)")
    #Iso, masym, deta
    Rdf = Rdf.Define("iso", "ruclu_energy / jetE")
    Rdf = Rdf.Define("ruclu_mass", "ruclu_energy * moe")
    ...
    branchList.push_back("nJets")
    branchList.push_back("nj_dr")
    branchList.push_back("nj_energy")
    branchList.push_back("ruclu_energy")
    branchList.push_back("ruclu_pt")
    branchList.push_back("ruclu_eta")
    branchList.push_back("ruclu_phi")
    branchList.push_back("ruclu_mass")
    branchList.push_back("diphoScores")
    branchList.push_back("monophoScores")
    branchList.push_back("hadronScores")
    branchList.push_back("moe")
    branchList.push_back("iso")
